...
public class LogFile {
    ...
    public void rollback(TransactionId tid)
        ...
        synchronized (Database.getBufferPool()) {
               rollback_helper(tid.getId());
            }
        }

    /* rollback_helper rollback the specified transaction by the long type.
    setting the state of any pages it updated to their pre-updated state.
    To preserve transaction semantics, this should not be called on transactions
    that have already committed (though this may not be enforced by this method.)

    @param long tid, the myid of the transactionId
    * */
    private void rollback_helper(long longTid) throws NoSuchElementException, IOException {
        synchronized (Database.getBufferPool()) {
            synchronized (this) {
                preAppend();
                // new codes
                Long recordId = tidToFirstLogRecord.get(longTid);
                if (recordId != null) {
        ...
        synchronized (Database.getBufferPool()) {
            synchronized (this) {
                ...
                if (lastCheckpoint > 0) {
                    ...
                    int activeCount = raf.readInt();
                    for (int i = 0; i < activeCount; i++) {
                        long tid = raf.readLong();
                        long recordId = raf.readLong();
                        uncommitted.add(tid);
                        tidToFirstLogRecord.put(tid, recordId);
                    }
                    raf.seek(raf.getFilePointer() + LONG_SIZE);
                }
                ...
                for (Long longTid : uncommitted) {
                    rollback_helper(longTid);
                }
                currentOffset = raf.getFilePointer();
            }
        }
